...
namespace EsWebDemo.Controllers
{
    public class HomeController : Controller
    {
        private readonly ElasticSearchSerivce<Article> _elasticSearchSerivce;

        public HomeController()
        {
            _elasticSearchSerivce = new ElasticSearchSerivce<Article>("article");
        }
        ...
        public async Task<ActionResult> Index(int? page)
        {
            ...
            await _elasticSearchSerivce.CreateIndexAsync();

            var pageSize = 10;
            var pageNumber = page ?? 1;
            var result = await _elasticSearchSerivce.Search((pageNumber - 1) * pageSize, pageSize);
        ...
        public async Task<ActionResult> Search(int? page, string filter, bool flag = false)
        {
            var pageSize = 10;
            var pageNumber = page ?? 1;
            ...
            {
                Filter = filter,
                SearchFields = new List<string>() { "title", "content", "descirption" },
                Skip = (pageNumber - 1) * pageSize,
                Size = pageSize
            };
            var result = await _elasticSearchSerivce.Search(input);

            ViewBag.filter = filter;
            ViewBag.took = result.Took;
            var paged = new StaticPagedList<Article>(result.List, pageNumber, pageSize, Convert.ToInt32(result.Total));
            ViewBag.paged = paged;
            ViewBag.flag = flag;
        ...
        {
            var result = await _elasticSearchSerivce.Search(prefix);
